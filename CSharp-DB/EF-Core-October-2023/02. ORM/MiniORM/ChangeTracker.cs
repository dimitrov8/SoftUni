    ...
    where T : class, new()
{
    ...
    {
        // Initialize lists for added and removed entities
        this.added = new List<T>();
        this.removed = new List<T>();
    }

    // Constructor with initial entities
    ...
    // Clones initial entities with allowed SQL types
    private static IList<T> CloneEntities(IEnumerable<T> originalEntities)
    {
        IList<T> clonedEntities = new List<T>();

        // Get PropertyInfo objects for properties with allowed SQL types
    ...
    // Get modified entities in a DbSet
    public IEnumerable<T> GetModifiedEntities(DbSet<T> dbSet)
    {
        var modifiedEntities = new List<T>();

        // Get PropertyInfo objects for primary key properties
        PropertyInfo[] primaryKeys = typeof(T).GetProperties()
            .Where(pi => pi.HasAttribute<KeyAttribute>())
            .ToArray(); // Get the properties that serve as primary keys

        // For each entity in the ChangeTracker's entity collection
        foreach (var proxyEntity in this.AllEntities)
        {
            // Get the values of the primary key properties for the current entity (proxyEntity)
            ...
            // Check if the "proxyEntity" is modified compared to the "originalEntity"
    ...
    // Check if there are modified properties between "proxyEntity" and "originalEntity"
    private static bool IsModified(T proxyEntity, T originalEntity)
    {
        // Get PropertyInfo objects for properties with allowed SQL types
        ...
        // Find modified properties by comparing proxyEntity and originalEntity
